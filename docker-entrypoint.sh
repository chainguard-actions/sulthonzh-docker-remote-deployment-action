...
if ! [ -z "${INPUT_COPY_STACK_FILE+x}" ] && [ $INPUT_COPY_STACK_FILE = 'true' ] ; then
  ...
  # Execute the deployment command using bash to interpret the pre-built command string
  # DEPLOYMENT_COMMAND is constructed from validated, controlled values (not user input)
  # INPUT_ARGS is validated by validate_input to reject shell metacharacters
  bash -c "${DEPLOYMENT_COMMAND} \"\$1\"" -- "$INPUT_ARGS" 2>&1
fi